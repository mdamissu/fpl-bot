...
const { getPlayerFormRank } = require("../../services/chartService");

module.exports = async function formRankHandler(interaction) {
    ...
    try {
        ...
        const ranks = getPlayerFormRank(player.id);
        if (!ranks || ranks.length === 0)
            return interaction.editReply("❌ No form rank data");

        execFile(
            "python",
            ["./handlers/chart/form_rank.py", playerName, JSON.stringify(ranks)],
            async (err, stdout) => {
                if (err) {
                    console.error(err);
                ...
                await interaction.editReply({
                    files: [{ attachment: buffer, name: `${playerName}_form_rank.png` }]
                });
            }
        );
